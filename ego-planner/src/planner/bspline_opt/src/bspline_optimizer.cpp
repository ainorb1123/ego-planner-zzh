...
namespace ego_planner
{
    namespace
    {
        double segmentYaw(const Eigen::Vector2d &from, const Eigen::Vector2d &to)
        {
    ...
    std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
    {
        ...
        if (!a_star_)
        {
            ROS_ERROR("BsplineOptimizer: a_star pointer is nullptr! Cannot perform path searching.");
        ...
        for (size_t i = 0; i < segment_ids.size(); i++)
        {
            ...
            for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
            {
                ...
                while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
                {
                    ...
                    int next_Astar_id = Astar_id + (val >= 0 ? -1 : 1);
                    if (next_Astar_id < 0 || next_Astar_id >= (int)a_star_pathes[i].size())
                        break;
                    Astar_id = next_Astar_id;
    ...
    bool BsplineOptimizer::check_collision_and_rebound(void)
    {
        ...
        if (flag_new_obs_valid)
        {
            ...
            {
                ROS_ERROR("BsplineOptimizer: a_star pointer is nullptr! Cannot perform path searching.");
                flag_new_obs_valid = false;  // 禁用新障碍物处理
            }
            else for (size_t i = 0; i < segment_ids.size(); ++i)
            {
                /*** a star search ***/
